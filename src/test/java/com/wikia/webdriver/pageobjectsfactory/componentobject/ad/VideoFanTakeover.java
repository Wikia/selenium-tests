...
public class VideoFanTakeover extends AutoplayVuap {
  ...
  public void clickOnAdImage() {
    runInAdFrame(() -> {
      ...
      new Actions(driver).moveToElement(driver.findElement(adImageTrigger), 10, 10)
          .click()
          .build()
          .perform();
  ...
  public double getAdVideoHeight() {
    return driver.findElement(By.cssSelector(String.format(UI_ELEMENT_SELECTOR_FORMAT, slotName)))
        .getSize()
        .getHeight();
  }
  ...
  public boolean isVideoAdBiggerThanImageAdMobile(double videoHeight, double imageHeight) {
    ...
    Log.log("isVideoAdBiggerThanImageAdMobile",
            "Ad image height = " + imageHeight + " is not smaller than video height = "
            + videoHeight,
            false,
            driver
    );
  ...
  public boolean isImageAdInCorrectSize(double imageHeight, String slotSelector)
    ...
    Log.log("isImageAdInCorrectSize",
            "Image is not in correct, required size " + imageHeight
            + " is not equal with actual size " + getAdSlotHeight(slotSelector),
            false,
            driver
    );
    return false;
  ...
  public boolean isTimeProgressing(double quartileTime, double midTime) {
    ...
    Log.log("isTimeProgressing",
            "Video time is not progressing, quartileTime " + quartileTime
            + " is not smaller than midTime " + midTime,
            false,
            driver
    );
    return false;
